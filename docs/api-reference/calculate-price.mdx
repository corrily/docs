...
Will override any previously saved price for a user and instead return a new price. Both price amount and currency will
be recomputed.

</Param>

<Param body="override_currency" type="boolean">

Will override the currency of any previously saved price and instead return the price in the currency provided. If set
to `true`, a `currency` param must be passed. Unlike `override` the price amount will *not* be recomputed.
...
<ResponseField name="products_to_select" type="enum">
In the case of a `packaging` experiment, the product API IDs to pick from the `product` object of the response to show
to users.
</ResponseField>
